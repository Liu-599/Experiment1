...
function qustionnaireRoutineBegin(snapshot) {
  return async function () {
    ...
    qustionnaireComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function qustionnaireRoutineEnd(snapshot) {
  return async function () {
    ...
    fetch('https://pipe.jspsych.org/api/data', {
      ...
      body: JSON.stringify({
        experimentID: 'zMM9kASDDWy6',  // ★ 修改为你自己的 experiment ID
        filename: filename,
        data: data,
      }),
    })
    ...
    .then(data => {
      ...
      // ✅ 上传成功后提示
      document.body.innerHTML = "<div style='font-size:20px;text-align:center;margin-top:30vh;'>数据上传成功，正在跳转问卷，请稍候...</div>";
    
      // ✅ 延迟 2 秒后跳转
      setTimeout(() => {
        window.location.href = "https://www.wjx.cn/vm/epfgAkP.aspx?"; // ★ 替换为你的问卷链接
      }, 2000);
    })
    .catch(error => {
